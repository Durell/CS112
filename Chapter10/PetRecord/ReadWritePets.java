...
public class ReadWritePets implements Serializable
{
  ...
  public static void main(String[] args)
  {
    ...
    while (!done)
    {
      ...
      if (userResponse.trim().toLowerCase().equals("w"))
      {
        ...
        if (fileObject.exists())
        {
          ...
          try
          {
            ...
            while (!doneAdding)
            {
              // we need a String name, int age, and double weight for each pet
        ...
        else
        {
          ...
          try
          {
            outputStream = new ObjectOutputStream(new FileOutputStream(fileObject));
            boolean doneAdding = false;
            while (!doneAdding)
            {
              System.out.println("Please give me the pet's name.");
      ...
      // handles case when user wants to read a file
      else if (userResponse.trim().toLowerCase().equals("r"))
      {
        ...
        try
        {
          ...
          {
          System.out.println("No more PetRecords in the file.");
          }
          inputStream.close();
        }
        ...
        {
        System.out.println("Problem with input from file " + userResponse);
        }
      }
    ...
    // STEP 2:
    // Determine oldest, youngest, biggest and smallest pets in file
    ObjectInputStream inputStream;
    try
    {
      int youngest = 10000;
      ...
      {
        System.out.println("No more pets in the file.");
      }
      inputStream.close();
